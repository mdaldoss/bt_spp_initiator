/* 
 * Example of a Bluetooth classic (Serial Profile Port) code of the initiator node
 * Usage: 
 *       Bluetooth acceptors names -> remote_device_name
 *       Set accordingly Number of BT acceptors (up to 7) -> SPP_ACCEPTOR_CNT      
 * 
 * Marco Daldoss - 2022
 * 
...
esp_bd_addr_t *peer_bd_addr = NULL;
static uint8_t peer_cnt = 0;
static uint8_t peer_bdname_len;
static char peer_bdname[ESP_BT_GAP_MAX_BDNAME_LEN + 1];
static const char *remote_device_name[SPP_ACCEPTOR_CNT] = {"BT-ACCEPTOR-1", "BT-ACCEPTOR-2"};
...
spp_acceptor_ctxt_t s_acceptors[SPP_ACCEPTOR_CNT] = {0};
uint8_t peer_idx = 0;
...
static void print_speed(uint32_t handle)
{
    float time_old_s = time_old.tv_sec + time_old.tv_usec / 1000000.0;
    float time_new_s = time_new.tv_sec + time_new.tv_usec / 1000000.0;
    float time_interval = time_new_s - time_old_s;
    float speed = data_num * 8 / time_interval / 1000.0;
    ESP_LOGI(SPP_TAG, "handle:%d speed(%fs ~ %fs): %f kbit/s" , handle, time_old_s, time_new_s, speed);
    data_num = 0;
...
static void init_acceptors(void)
{
    memset(s_acceptors, 0, sizeof(s_acceptors));
}

static spp_acceptor_ctxt_t *find_acceptor_by_handle(uint32_t handle)
{
    spp_acceptor_ctxt_t * acceptor = NULL;

    for (size_t i = 0; i < SPP_ACCEPTOR_CNT; i++) {
        if (s_acceptors[i].handle == handle) {
            acceptor = &s_acceptors[i];
            break;
        }
    }
...
static void esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t *param)
{
    ...
    case ESP_SPP_DISCOVERY_COMP_EVT:
        if (param->disc_comp.status == ESP_SPP_SUCCESS) {
            ...
            esp_spp_connect(sec_mask, role_master, param->disc_comp.scn[0], s_acceptors[peer_idx].addr);
            peer_bd_addr = &s_acceptors[peer_idx].addr;
        } else {
    ...
    case ESP_SPP_OPEN_EVT:
        if (param->open.status == ESP_SPP_SUCCESS) {
            ...
            s_acceptors[peer_idx].handle = param->open.handle;
            s_acceptors[peer_idx].p_data = spp_data;
            peer_idx++;
            if (peer_idx < SPP_ACCEPTOR_CNT) {
                esp_spp_start_discovery(s_acceptors[peer_idx].addr);
            }
    ...
    case ESP_SPP_WRITE_EVT:
        acceptor = find_acceptor_by_handle(param->write.handle);
        if (!acceptor) {
            ESP_LOGE(SPP_TAG, "not find the connected acceptor, handle:%d", param->write.handle);
            break;
        }

        if (param->write.status == ESP_SPP_SUCCESS) {
            if (acceptor->p_data + param->write.len == spp_data + SPP_DATA_LEN) {
                /* Means the previous data packet be sent completely, send a new data packet */
                acceptor->p_data = spp_data;
            } else {
                /*
                 * Means the previous data packet only be sent partially due to the lower layer congestion, resend the
                 * remainning data.
                 */
                acceptor->p_data += param->write.len;
            }
#if (SPP_SHOW_MODE == SPP_SHOW_DATA)
            /*
            ...
            if (time_new.tv_sec - time_old.tv_sec >= 3) {
                print_speed(param->write.handle);
            }
#endif
        } else {
        ...
        if (!param->write.cong) {
            ...
            esp_spp_write(param->write.handle, spp_data + SPP_DATA_LEN - acceptor->p_data, acceptor->p_data);
        } else {
            /*
    ...
    case ESP_BT_GAP_DISC_RES_EVT:
        ...
        for (int i = 0; i < param->disc_res.num_prop; i++){
            if (param->disc_res.prop[i].type == ESP_BT_GAP_DEV_PROP_EIR
                ...
                if (peer_cnt < SPP_ACCEPTOR_CNT && strlen(remote_device_name[peer_cnt]) == peer_bdname_len &&
                    strncmp(peer_bdname, remote_device_name[peer_cnt], peer_bdname_len) == 0) {
                    memcpy(s_acceptors[peer_cnt].addr, param->disc_res.bda, ESP_BD_ADDR_LEN);
                    printf("device_name:%s rem_bda:[%s]\n", remote_device_name[peer_cnt], bda2str(s_acceptors[peer_cnt].addr, bda_str, sizeof(bda_str)));
                    peer_cnt++;
                }
            }
        }

        if (peer_cnt == SPP_ACCEPTOR_CNT) {
            peer_idx = 0;
